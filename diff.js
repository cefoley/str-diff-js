...
function assertMatrix(expected, a, b) {
	...
	let message = `Matrix for '${a}' and '${b}' was ${actual.join("|")}.`;
...
function editMatrix(a, b) {
	...
	for (var r = 0; r < rows; r++) result[r] = new Array(cols);
	for (var c = 0; c < cols; c++) result[0][c] = c;
	for (var r = 1; r < rows; r++) result[r][0] = 1;
	for (var r = 1; r < rows; r++) {
		for (var c = 1; c < cols; c++) {
			result[r][c] = (a[r]==b[c] ? 0 : 1);
		}
	}
	return result;
}